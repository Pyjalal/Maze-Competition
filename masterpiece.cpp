...
bool isStuck = false;
bool visited[MAZE_SIZE][MAZE_SIZE] = {false}; // To prevent loops

/***** Dijkstra’s Algorithm Storage *****/
int distanceGrid[MAZE_SIZE][MAZE_SIZE];  // Distance grid computed by Dijkstra’s algorithm
int backtrackX[100], backtrackY[100];      // For backtracking
int backtrackIndex = 0;
int previousBestDistance = 9999; // For EEPROM saving criteria

/***** Q-Learning Variables *****/
float Q_table[MAZE_SIZE][MAZE_SIZE][4] = {{{0}}};  
float LEARNING_RATE = 0.5;       // Decays over time
const float MIN_LEARNING_RATE = 0.1;
const float DISCOUNT_FACTOR = 0.9;
float EXPLORATION_RATE = 0.5;    // ε for exploration (50% initially)
...
/***** Motor & Encoder Setup *****/
// Use your provided motor control pins and functions:
...
/***** IMU Setup & Functions *****/
// MPU6050 Setup using provided code
...
void updateQTable(int prevX, int prevY, int action, float reward) {
  ...
  float maxNextQ = Q_table[posX][posY][bestNextAction];
  Q_table[prevX][prevY][action] += LEARNING_RATE * (reward + DISCOUNT_FACTOR * maxNextQ - Q_table[prevX][prevY][action]);
...
float distanceArr[3] = {0, 0, 0};  // For left, right sensors
...
bool isTurnRight = false, isTurnLeft = false, isUTurn = false;
const byte mazeWidth = 20;
float distanceArr[3] = {0, 0, 0}; // For left, right distances
void moveForwardAfterTurn(){
    current = millis();
    while(true){
        update();
